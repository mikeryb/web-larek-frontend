...
- `preview:selected` - изменение открываемой в модальном окне карточки товара. Событие происходит при выборе товара для предпросмотра

События класса `CartData`
- `cart:changed` - возникает при любом изменении состава корзины

События класса `OrderData`
- `order:changed` - возникает при сабмите в формах контактных или адресных данных
- `order:cleared` - возникает при оформлении заказа

События класса `Card`
- `card:add` - возникает при добавлении товара в корзину
- `card:delete` - возникает при удалении товара из корзины
...
- Пользователь нажимает кнопку «В корзину» в карточке товара (Card)

- Компонент генерирует событие card:add, передавая объект товара
